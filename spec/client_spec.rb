...
RSpec.describe Sudrel::NfseSdk::Client do
  ...
  it "obter_conta faz GET /v1/account com o Bearer correto" do
    ...
    client = described_class.new(api_key: "sdr_live_abc", transporte: transporte)
    ...
    expect(headers["Authorization"]).to eq("Bearer sdr_live_abc")
  end

  it "listar_empresas devolve o array direto (sem paginação)" do
    transporte = ->(*) { resposta(200, [{ "id" => "1" }, { "id" => "2" }]) }
    client = described_class.new(api_key: "sdr_test_abc", transporte: transporte)
  ...
  it "lança ApiError com status_code e mensagem no erro" do
    ...
    expect { client.obter_empresa("inexistente") }.to raise_error(Sudrel::NfseSdk::ApiError) do |erro|
